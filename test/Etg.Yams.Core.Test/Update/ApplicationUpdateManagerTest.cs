...
namespace Etg.Yams.Test.Update
{
    public class ApplicationUpdateManagerTest
    {
        ...
        public async Task TestMultipleUpdates()
        {
            ...
            IDeploymentStatusManager deploymentStatusManagerStub = new StubIDeploymentStatusManager()
                .UpdateDeploymentStatusAtomically(updateAction =>
                {
                    updateAction(deploymentStatus);
            ...
            Assert.Equal(3, deploymentStatus.ListAll().Count());
            VerifyThatDeploymentStatusHasBeenUpdated(deploymentStatus, app1v3, ClusterId, InstanceId);
            VerifyThatDeploymentStatusHasBeenUpdated(deploymentStatus, app1v4, ClusterId, InstanceId);
            VerifyThatDeploymentStatusHasBeenUpdated(deploymentStatus, app1v5, ClusterId, InstanceId);
        }

        private void VerifyThatDeploymentStatusHasBeenUpdated(DeploymentStatus deploymentStatus, 
            AppIdentity appIdentity, string clusterId, string instanceId)
        {
            var appDeploymentStatus = deploymentStatus.GetAppDeploymentStatus(clusterId, instanceId, appIdentity);
